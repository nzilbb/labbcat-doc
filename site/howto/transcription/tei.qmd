...
## How to use TEI texts for upload
...
Other tags are by default mapped on to the *entities* LaBB-CAT layer (in which case their tag name, and its type attribute if present, will be used for the entity label).  Alternatively, if there is a LaBB-CAT layer that is named after the TEI tag name, then tags will be mapped to that layer by default during upload. For example, to have all `<sic>` tags extracted to their own layer (instead of the *entities* layer) by default, create a new 'phrase' layer called *sic*.
