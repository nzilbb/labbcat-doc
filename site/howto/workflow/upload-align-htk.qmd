...
: If you click the word, it will open, in a separate tab, the first transcript in which the word appears, so you can see the context, play the audio, etc.\
  ...
  - If the word is a spelling mistake, you need to correct it in the transcript:
     ...
     1. fix the spelling error:\
        ![](transcript-correction.png){fig-alt="A window showing the original utterance above, 'so it's a fair bit of histoy . yeah' and the corrected version in a text box below: 'so it's a fair bit of history . yeah'"}\
     1. press *Update* and then *Close*.
  - In some cases, it's an interruped word, or some other one-off case that shouldn't be added to the global dictionary, because it won't come up over and again in other transcripts.\
    These cases need to have a *pronounce* tag added to the word token directly in the transcript:
     1. click the word to open the transcript where it appears,
     1. click the word in the transcript and select the *Edit* (the last option, under 'Word') from the resulting menu,\
        (this opens a window that shows all the annotation layers for that word token),\
        ![](add-pronounce-tag.png){fig-alt="A box for the word token 'pres~', showing each annotation layer. The 'pronounce' layer's box has already been filled in with the pronunciation 'pr@z'"}
     1. in the *pronounce* box, enter the pronunciation of the word using CELEX's DISC symbols,
     1. Press the tick button, or press the {{< kdb Enter >}} key to save
- A search button
