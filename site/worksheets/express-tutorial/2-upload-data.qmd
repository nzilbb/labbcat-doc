...
We want to upload no only the transcript, but also its associated media files.
...
(@) Press *Choose Files* button again, and in the same "QuakeStories" folder 
    ...
    the {{< kbd win=Ctrl linus=Ctrl mac=command >}} key on your keyboard and click the file called
...
(@) Most of the transcripts we are going to upload are monologues, so set in the *Defaults* box
