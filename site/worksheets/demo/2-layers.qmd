...
(@) Delete the `hit|struck` test from the *orthography* box above.
...
You can also export segment token that match search patterns to a CSV file, including the start/end times
...
You'll see the CSV file includes the columns "Target segment start" and
"Target segment end"; these columns have the start and end time of the
matching 'TRAP' vowel token, and can be used for calculating vowel duration, 
...
(@) In LaBB-CAT, click the *upload* menu option.
(@) Click the *process with praat* option.
(@) Click *Browse* and select the CSV results that you saved at the end of the last exercise,\
    i.e. word-intial tokens of /æ/ followed by /n/.
    You will see a form to fill in, and the first couple of settings
    (*Transcript Name column* and *Participant column* should be already
    filled in).
(@) For the *Start Time column*, ensure that the *Target segment start*
...
-   Annotation layers can be searched using the search matrix.<!--, using
    numeric value or regular expressions, depending on the type of layer.-->
-   Layers can be optionally displayed in transcripts.
-   words can be tagged with their frequency in the corpus itself, or in the COBUILD corpus,
