...
(@) If you have Praat installed on your computer, 
    select the *Praat Text Grid* option on the *Export* tab. Save the resulting
    file on your desktop, and then open it with Praat.\
    You will see that the TextGrid has a couple of tiers, one for each speaker
    with utterance transcripts.
...
(@) Click the *Attributes* link for the main participant.\
    This will display the participant meta-data, except that there are no values 
    because we haven't entered them yet!
...
(@) In LaBB-CAT, select the *participants* option on the menu.\
    You will see all the participants in the the transcripts you have uploaded,
    many of which include an interviewer as well as a main participant.
...
(@) Select the *participants* option on the menu.
    ...
    and you will see that now the speakers who are main-participants have values
    filled in for their participant attributes.
...
(@) Press *New* to add the task.\
    The task will be created and you'll be shown a page titled "define elicitation steps for task"
...
(@) Click the participant's *Attributes* icon (the spanner/wrench on the right) 
    to open their attributes page, and check
