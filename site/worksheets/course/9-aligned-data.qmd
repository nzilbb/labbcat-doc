...
(@) Select the *speechRate* option in this dropdown list.\
    You will see a progress bar while the statistics for the transcript
    are computed again.
(@) Once that's finished, the transcript will reload itself.\
    Check the speech rate annotations again. You should see that at least 
    some of them are different; the rates will be higher because the inter-word
    pauses are now excluded from the total time of the speech.
...
(@) We want to search for all instances of the *FLEECE* vowel, so use
    ...
    If you used MFA for forced alignment, this may be `iː`
...
(@) To make sure we only get words that have been aligned by HTK/MFA or
    manually, in the *Options* tab,
    tick the *Only match words that are aligned* option.
(@) Press *Search*.\
...
(@) On the *Layers* tab, tick the *formants* project option.\
    This will reveal the (empty) *F1* layer in the *Segments* column in the list
    of layers.
(@) Tick the *F1* layer.\
...
(@) Click on the first search result (which is highlighted in yello).
...
(@) Click *Choose File* and select the CSV results that you saved
    ...
    filled in.
(@) For the *Start Time column* ensure that the *segment start*
    option is selected.
(@) For the *End Time column* ensure the *segment end* option is
    selected.\
...
(Some acoustic measurements require more audio context, so if you find there
are values missing, chang the context to be more than 0.025s, and you should
find the measurement is successful.)
...
HTK/MFA alignments, and to check and possibly manually-correct those
alignments.
