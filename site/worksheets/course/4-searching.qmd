...
(@) In LaBB-CAT, select the *search* option on the menu.\
    This takes you to a page with a number of tabs above
...
(@) Open the *Participants* tab at the top left.
(@) Press the *Filter Participants* button.\
...
(@) We want the participants who speak only English, so enter `English`
    ...
    *English* in their languages. It also includes participants who
    speak additional languages, and we want to eliminate these.\
    ...
    `^English$` means, "English at the beginning (with nothing else before), 
    and at the end (with nothing else after)"
(@) Press the *Layered Search* button at the top of the list.\
    You will be returned to the search page. 
    Now the participants you selected listed at the top.
...
(@) Each match is highlighted and shown with some context (the previous
    ...
    Select *up to 5 words* to see more context around each match.
(@) Click on the first match.\
    You will see that the interactive transcript page opens in a new
    tab, with the match at the top, and highlighted in yellow. You will
...
(@) Now go back to the results page and press the expand `▾` button to the right 
    of the *Audio Export* button, to reveal further options. 
