...
(@) We're going to search all male monolinguals, so use the *Participants* tab
    as before to filter by the appropriate participant attribute values.
...
(@) Click *Insert Annotations*.\
    ...
    tagged in LaBB-CAT with either "TP" or "FP"
...
(@) In LaBB-CAT, select the *search* menu option. If we don't filter by participant
    attributes, as we did before, the search page searches utterances of all participants,
    which is what we're going to do now.\
    This time we're going to search for the true-positive annotations
    we just inserted.
(@) There's now a "Projects" to the left of the layer checkboxes
    which includes the "the" project we added at the start. Tick
...
(@) Listen carefully to see whether the speaker pronounces the word
    ...
    the code `@`.
(@) Hover your mouse pointer over the `TP` tag above the matched word, and you will see
    a small button appear with the icon of a pencil on it ![](edit-token.png). 
    Hover your mouse pointer over the button to see what it does, and then press it.
    The `TP` tag will now appear in a text box that allows you to change the label.
(@) We're going to change the "TP" annotation depending on the
    pronunciation of the token. So replace "TP" with `i` or `@` as
    appropriate.
(@) Press the *Save* button ![](save-token.png) to save your annotation
    to the database.
(@) You will see an alert asking if you're sure. Press *OK*.
