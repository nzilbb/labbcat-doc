...
you can see the underlying DISC characters by disabling the 'interpretation'
option on the layer in the transcript.

(@) On the *Layers* tab, to the right of the name of the *phonemes* layer, there is
a small 'interpretation' ![](interpretation-selector.png) selector.\
...
(@) Hover your mouse pointer over the *phonemes* tag you can see.\
    You will see that above
    the pronunciation that starts with /w/, another annotation appears
    that starts with /h/. 

This word has multiple *phonemes* tags and hovering the mouse reveals all of them.

Only the first one is displayed in the transcript by default, but when you do
